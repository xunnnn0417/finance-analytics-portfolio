...
我原本的學習背景偏向生醫領域，訓練過程很重視觀察、證據、變因控制與資料判讀。接觸金融市場後，我發現經濟與市場決策其實也很需要這種思考方式：不能只看感覺，也不能只看一次結果，而是要用資料去驗證自己的判斷。

所以我開始用 Python 整理市場資料、建立回測流程，並用壓力測試與樣本外驗證檢查策略是否穩定。這份報告不是為了證明我「會交易」，而是想呈現我如何面對不確定資料、建立模型、比較結果，並用風險意識檢查結論。
...
我一開始想知道的是：如果在基本策略上加入趨勢或 regime 濾網，結果會不會更穩？是報酬變好，還是交易次數變少，或者只是看起來比較漂亮而已？
...
Hurst 濾網在這次測試中提高了最佳組合的報酬，HMM proxy 則明顯減少交易次數並降低部分回撤。不過我不會把這個結果直接解讀成「濾網一定有效」，因為它還需要更多期間與不同市場驗證。這個案例讓我理解到：模型不是越複雜越好，而是要看它是否能在風險與交易機會之間取得合理平衡。

## 案例二：US100 資料清理與嚴格壓力測試

### 資料工程

我使用 Python 從 Dukascopy 下載 US100 一分鐘資料，並檢查資料完整性。這部分對我來說很重要，因為如果資料本身有問題，後面的回測結果就沒有太大意義。
...
這個案例的重要性不在於找到高報酬策略，而是建立「不通過也要如實記錄」的研究習慣。結果顯示多數候選在重成本壓力下失敗，表面上不好看，但我認為這反而是有價值的地方。對經濟與金融分析來說，避免被漂亮績效誤導，比追求單一最佳結果更重要。
...
全樣本結果看起來亮眼，但我再看 walk-forward 報告時，發現結果並沒有那麼穩定：
...
這個案例提醒我，金融資料分析不能只看全樣本最佳化結果。若樣本外或 walk-forward 不穩定，就必須保留疑問，不能直接下過度樂觀的結論。對我來說，能夠看出結果的限制，也是分析能力的一部分。
...
這些作品讓我理解到，市場分析不是追求單一漂亮數字，而是建立一套能被檢查的研究流程：先取得資料，再清理資料，接著建立假設與模型，最後用壓力測試、成本假設與樣本外驗證檢查結論。

這樣的自學訓練讓我更想進入經濟學系。因為我目前能做的是用工具與程式去整理資料、測試想法，但我也知道自己還需要更完整的經濟學基礎。未來我希望系統性學習個體經濟、總體經濟、統計、計量經濟與金融市場，讓自己的分析不只停留在技術操作，而能連結更完整的經濟理論與決策框架。
